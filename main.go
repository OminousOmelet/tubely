...
func main() {
	...
	// client from AWS bucket
	aws, err := config.LoadDefaultConfig(context.Background(), config.WithRegion(cfg.s3Region))
	if err != nil {
		log.Fatal("Failed to create AWS client")
	}
	cfg.s3client = s3.NewFromConfig(aws)
